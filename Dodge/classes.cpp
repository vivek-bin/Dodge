...
public:
    ...
    void Add(shape_Object* const Data){
        Node *x=new Node;
        x->Obj=Data;
        x->Next=Top;
        Top=x;
    ...
    shape_Object* popMid(void *ParentOfX){
        Node *x;
        if(ParentOfX==NULL || getNext(ParentOfX)==NULL){
            return NULL;
        }
        shape_Object* Data;
        x=((Node*)ParentOfX)->Next;
        Data=x->Obj;
        ((Node*)ParentOfX)->Next=x->Next;
        delete x;
        return Data;
    }
    ...
    void* getNext(void* x){
        if(x==NULL)
            return NULL;
        return (void*)((Node*)x)->Next;
    }

    shape_Object* getData(void* x){
        if(x==NULL)
            return NULL;
        return ((Node*)x)->Obj;
    }
...
class small_Queue{
private:
    shape_Object* Data[25];
    short Size;
    short Front,Back;
    short Invalid(){return -1;}
public:
    small_Queue(short QSize){
        Size=QSize;
        Front=Back=Invalid();
        for(int i=0;i<25;++i)Data[i]=NULL;
    }

    void addElement(shape_Object* Val){
        if(Front==Invalid()){
            Front=Back=0;
            Data[Back]=Val;
        }
    ...
    void removeElement(shape_Object* Val){
        if(Front==Invalid())return;
        if(Front==Back){
            if(Data[Front]==Val){
                Data[Front]=NULL;
                Front=Back=Invalid();
    ...
    short getNext(short I){
        if(Front==Invalid()
        || I==Back
        || I<0
        || I>=Size
        || (Front<Back && (I>Back || I<Front))
        || (Front>Back && (Front>I && I>Back)))
            return Invalid();

        return (I+1)%Size;
    }

    shape_Object* getData(short I){
        if(Front==Invalid()
        || I<0
        || I>=Size
        || (Front<Back && (I>Back || I<Front))
    ...
    short numElements(){
        return (Front==Invalid())? 0 : ((Back-Front+Size)%Size)+1;
    }
